...
const MARKER: &str = ".shadow-batch-temp";
...
impl TempJob {
    pub fn create() -> Result<Self> {
        ...
        fs::write(dir.join(MARKER), b"shadow-batch-processor")?;
        Ok(Self { dir })
    }
...
pub fn cleanup_stale() -> Result<()> {
    ...
    let entries = fs::read_dir(&root)
        .map_err(|err| Error::detailed("Could not inspect temporary files.", err.to_string()))?;
